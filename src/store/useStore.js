  ...
  subscribeToProject: (projectId) => {
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const exists = get().messages.find(m => m.id === payload.new.id)
        if (!exists) set(s => ({ messages: [...s.messages, payload.new] }))
      })
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        ...
        set(s => ({ quotes: s.quotes.map(q => q.id === payload.new.id ? payload.new : q) }))
      })
      .subscribe()
